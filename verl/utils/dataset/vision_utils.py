...
class MissingImageError(FileNotFoundError):
    """Raised when a referenced image file cannot be located locally."""

    def __init__(self, uri: str | None = None):
        self.uri = uri
        message = f"Image not found: {uri}" if uri else "Image not found"
...
def process_image(image: dict | Image.Image) -> Image.Image:
    ...
    if isinstance(image, dict):
        source_description = image.get("image") or image.get("path") or image.get("url")
    ...
    try:
        ...
        if source_description is None:
            source_description = repr(image)
        logger.warning("Image unavailable during preprocessing, dropping sample: %s", source_description)
        raise MissingImageError(source_description) from exc
