    ...
    def _preprocess_reward_inputs(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            if self.model_type == "discriminative":
                ...
                preprocess_outputs = self.preprocess_fn(
                    rollout_question=rollout_question,
                    rollout_response=rollout_response,
                    ground_truth=ground_truth,
                )
                if not isinstance(preprocess_outputs, (list, tuple)):
                ...
                rm_input = [
                    ...
                    for output_str in preprocess_outputs
                ]

            if isinstance(rm_input, list) and rm_input and isinstance(rm_input[0], list):
                count = len(rm_input)
                input_counts.append(count)
                rm_inputs.extend(rm_input)
            else:
                input_counts.append(1)
                rm_inputs.append(rm_input if isinstance(rm_input, list) else rm_input)
        ...
        return rm_inputs, input_counts

    def _postprocess_reward_outputs(
        self,
        data: DataProto,
        output: list[float] | list[list[int]],
        input_counts: list[int],
        ...
        if self.model_type == "discriminative":
            ...
            scores: list[float] = []
            postprocess_sig = signature(self.postprocess_fn)
            accepts_meta = len(postprocess_sig.parameters) >= 2
            idx = 0
            for sample_idx, count in enumerate(input_counts):
                group = output_text[idx : idx + count]
                idx += count
                if count == 1:
                    group_input = group[0]
                else:
                    group_input = group
                if accepts_meta and sample_idx < len(self._sample_meta):
                    meta = self._sample_meta[sample_idx]
                    score = self.postprocess_fn(group_input, meta)
                else:
                    score = self.postprocess_fn(group_input)
                scores.append(float(score))
            scores = torch.tensor(scores)

        token_level_scores = self._expand_to_token_level(data, scores)
        self._sample_meta = []
        return token_level_scores, scores

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="reward_model"))
    @DistProfiler.annotate(color="brown")
    def compute_rm_score(self, data: DataProto):
        data = data.to("cpu")
        rm_inputs, input_counts = self._preprocess_reward_inputs(data)

        output = self.reward_model.compute_reward(rm_inputs)
        token_level_scores, sample_scores = self._postprocess_reward_outputs(data, output, input_counts)
        # Note that this is only the scores, may not be the final rewards used to train RL
        output = DataProto.from_dict(
            tensors={"rm_scores": token_level_scores},
            non_tensors={"rm_scalar_scores": sample_scores.detach().cpu().tolist()},
        )
        return output
