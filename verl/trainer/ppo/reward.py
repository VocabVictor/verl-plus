...
from verl.reward import build_reward_specs
...
def load_reward_manager(
    ...
    reward_manager_name = config.reward_model.get("reward_manager", "rule")
    ...
    if compute_score is None:
        ...
        if sandbox_url:
            raise ValueError(
                "Sandbox fusion requires a custom reward implementation."
                " Provide reward_model.custom_reward_function instead of relying on default_compute_score."
            )

    reward_model_cfg = config.reward_model
    reward_kwargs = dict(reward_model_cfg.get("reward_kwargs", {}))

    reward_functions = list(reward_model_cfg.get("reward_funcs") or [])
    reward_weights = reward_model_cfg.get("reward_weights")

    reward_config_mapping = None
    if reward_functions:
        reward_config_mapping = OmegaConf.to_container(reward_model_cfg, resolve=True)
        reward_specs = build_reward_specs(reward_functions, config=reward_config_mapping)
        if reward_specs:
            reward_kwargs.setdefault("reward_specs", reward_specs)
        if reward_config_mapping is not None:
            reward_kwargs.setdefault("reward_config", reward_config_mapping)

    if reward_weights:
        reward_kwargs.setdefault("reward_weights", reward_weights)
