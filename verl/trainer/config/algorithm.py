...
class FilterGroupsConfig(BaseConfig):
    ...
    Args:
        ...
        mode (str): Filtering mode. "std" keeps the original DAPO behaviour
            (only drop identical groups). "mean_threshold" filters by metric
            mean.
        mean_min (Optional[float]): Lower bound for mean when mode is
            "mean_threshold". `None` means no lower bound.
        mean_max (Optional[float]): Upper bound for mean when mode is
            "mean_threshold". `None` means no upper bound.
        warmup_steps (int): Number of global training steps to skip filtering.
    """
