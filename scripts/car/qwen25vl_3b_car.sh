...
OUTPUT_DIR="${OUTPUT_DIR:-$WORKDIR/outputs/mm_eruka_qwen25vl_3b_caption}"
LOG_DIR="${LOG_DIR:-$WORKDIR/logs}"
QUESTION_CACHE_PATH="${QUESTION_CACHE_PATH:-$OUTPUT_DIR/caption_question_cache.jsonl}"
mkdir -p "$LOG_DIR" "$(dirname "$QUESTION_CACHE_PATH")"
...
export OPENAI_API_KEY="${OPENAI_API_KEY:-sk-kVuL6bN0yrUmqSDibPjuYlsR0FAw2afp1mKNcRFWCbj0Nau2}"
export OPENAI_BASE_URL="${OPENAI_BASE_URL:-https://yunwu.zeabur.app/v1}"
export WANDB_MODE="${WANDB_MODE:-online}"
export PRECOMPUTE_QUESTION_CACHE="${PRECOMPUTE_QUESTION_CACHE:-true}"

# Precompute question cache if enabled
if [[ "$PRECOMPUTE_QUESTION_CACHE" == "true" ]]; then
    echo "Precomputing question cache before training..."
    if [[ ! -f "$QUESTION_CACHE_PATH" ]]; then
        cd "$WORKDIR"
        python precompute_question_cache.py
        echo "Question cache precomputation completed."
    else
        echo "Question cache already exists at $QUESTION_CACHE_PATH"
    fi
fi
...
OVERRIDES=(
  ...
  "reward_model.reward_weights=[1.0,1.0,0.1]"
  ...
  "reward_model.reward_kwargs.caption.max_concurrency=256"
  "+reward_model.reward_kwargs.caption.question_cache_path=${QUESTION_CACHE_PATH}"
  ...
  "trainer.experiment_name=mm_eruka_qwen25vl_3b_caption"
...
python -m verl.trainer.main_ppo "${OVERRIDES[@]}" "$@" 2>&1 | tee "$LOG_FILE"
