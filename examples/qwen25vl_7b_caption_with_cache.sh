...
OUTPUT_DIR="${OUTPUT_DIR:-$WORKDIR/outputs/mm_eruka_qwen25vl_7b_caption}"
...
# 设置缓存路径 (与3B共用同一份缓存)
QUESTION_CACHE_PATH="$WORKDIR/outputs/mm_eruka_qwen25vl_3b_caption/caption_question_cache.jsonl"
...
OVERRIDES=(
  ...
  # 框架级缓存配置
  "+data.caption_cache_path=${QUESTION_CACHE_PATH}"
  "+data.caption_cache_config.base_url=${OPENAI_BASE_URL}"
  "+data.caption_cache_config.api_key_env=OPENAI_API_KEY"
  "+data.caption_cache_config.model=${OPENAI_MODEL_NAME:-gpt-4o-mini}"
  "+data.caption_cache_config.max_concurrency=64"
  "+data.caption_cache_config.timeout=120"
  ...
  # CaptionReward配置
  "reward_model.reward_kwargs.caption.api_key=${OPENAI_API_KEY}"
  "reward_model.reward_kwargs.caption.base_url=${OPENAI_BASE_URL}"
  "reward_model.reward_kwargs.caption.model=${OPENAI_MODEL_NAME:-gpt-4o-mini}"
  "reward_model.reward_kwargs.caption.max_concurrency=64"
  ...
  "reward_model.reward_kwargs.caption.timeout=120"

  # 使用纯 rule-based reward，不需要 gain/sglang
  ...
  "trainer.experiment_name=mm_eruka_qwen25vl_7b_caption"
